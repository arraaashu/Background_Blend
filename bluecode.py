...
while(capt.isOpened()):
    ...
    #For masking the color we have taken initially blue to be covered.
    #So, the hsv lower and higher values are to be given inorder to blend the 
    #background perfectly.
    lblue = np.array([80, 50, 50])
    ublue = np.array([80, 255,255])

    blend1 = cv2.inRange(hsv, lblue, ublue)

    lblue = np.array([105, 70, 40])
    ublue = np.array([115, 255, 255])


    blend2 = cv2.inRange(hsv, lblue, ublue)
    ...
    #creating an inverted blend to segment out the blue color from the frames.
    blend2 = cv2.bitwise_not(blend1)
    #segment the blue color part out of the frame using bitwise and with the inverted blend.
