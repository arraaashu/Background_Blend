...
while(capt.isOpened()):
    ...
    #For masking the color we have taken initially green to be covered.
    #So, the hsv lower and higher values are to be given inorder to blend the 
    #background perfectly.
    lgreen = np.array([100, 50, 50])
    ugreen = np.array([100, 255,255])

    blend1 = cv2.inRange(hsv, lgreen, ugreen)

    lgreen = np.array([30, 50, 50])
    ugreen = np.array([102, 255, 255])


    blend2 = cv2.inRange(hsv, lgreen, ugreen)
    ...
    #creating an inverted blend to segment out the green color from the frames.
    blend2 = cv2.bitwise_not(blend1)
    #segment the green color part out of the frame using bitwise and with the inverted blend.
